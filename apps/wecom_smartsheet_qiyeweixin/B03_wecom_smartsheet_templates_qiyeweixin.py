"""
程序简介：读取企业微信智能表格字段和样例数据，辅助生成排产模板或调试字段结构。
...
# ==========================================================
MODE = 3  # 新增模式说明：
# 1 = 新建并存储；0 = 刷新 registry 里所有表的ID并打印变更
# 2 = 基于模板创建新文档 + 获取实例ID + 同步到registry
# 3 = 获取所有模板ID列表（打印+存入registry）

# MODE=1 时使用
DOC_NAME = "生产任务排期"
SHEET_TITLES = ["排产·统计总台账", "标准型号规格表"]

# MODE=2 时使用（基于模板创建）
TEMPLATE_ID = "你的模板ID"  # 替换成你的模板ID（提前通过MODE=3获取）
TEMPLATE_CREATE_DOC_NAME = "生产任务排期_20260125"  # 基于模板创建的新文档名称

# MODE=0 时可选：不填=自动刷新 registry 里全部 docid；填了=只刷新该 docid
TARGET_DOCID = ""

# 本地索引文件：给其它程序调用（docid / sheet_id / template_id 都在这里）
...
# =========================
# 2) Token / Doc / Sheet / Template API（新增模板相关接口）
# =========================
...
# 新增：模板相关核心接口
# =========================
def get_all_templates(access_token: str) -> list[dict]:
    """
    获取企业微信智能表格所有模板列表（包含模板ID）。
    返回格式：[{"template_id": "...", "template_name": "...", "create_time": "...", ...}, ...]
    """
    # 企业微信模板列表接口（适配最新版API）
    payload = {
        "offset": 0,
        "limit": 100  # 单次最多100条，足够大部分场景
    }
    data = api_post("/wedoc/template/list", {"access_token": access_token}, payload)
    if data.get("errcode") != 0:
        raise RuntimeError(f"get_all_templates failed: {data}")

    templates = data.get("template_list") or []
    # 标准化模板数据
    normalized_templates = []
    for tpl in templates:
        normalized_templates.append({
            "template_id": tpl.get("template_id") or tpl.get("id") or "",
            "template_name": tpl.get("template_name") or tpl.get("name") or "",
            "create_time": tpl.get("create_time") or "",
            "creator": tpl.get("creator") or "",
            "raw": tpl
        })
    return normalized_templates


def create_doc_from_template(access_token: str, template_id: str, doc_name: str, admin_users: list[str]) -> dict:
    """
    基于模板创建新的智能表格文档。
    返回：包含新文档的docid、url等信息
    """
    payload = {
        "template_id": template_id,
        "doc_name": doc_name,
        "admin_users": admin_users,
        "doc_type": 10  # 10=智能表格（固定值）
    }
    data = api_post("/wedoc/create_doc_from_template", {"access_token": access_token}, payload)
    if data.get("errcode") != 0:
        raise RuntimeError(f"create_doc_from_template failed: {data}")
    return data


# =========================
# 3) Registry：本地索引文件（新增模板ID存储）
# =========================
def load_registry(path: str) -> dict:
    """读取 registry（没有就返回空）。"""
    if not os.path.exists(path):
        return {"templates": {}, "docs": {}, "sync_errors": []}  # 新增templates节点
...
def ensure_template_entry(registry: dict, template_id: str, template_name: str, raw: dict = {}) -> None:
    """确保 registry 里存在模板ID条目"""
    ...
    if template_id not in templates:
        ...
        templates[template_id]["last_sync_at"] = now_str()
        templates[template_id]["template_name"] = template_name  # 更新最新名称
        templates[template_id]["raw"] = raw
...
# 4) 新增模式：模板相关处理
# =========================
def run_mode_get_templates(token: str) -> None:
    """MODE=3：获取所有模板ID列表，打印并存储到registry"""
    print("=== 获取模板列表开始 ===")
    ...
    for idx, tpl in enumerate(templates, 1):
        ...
        ensure_template_entry(registry, tpl_id, tpl_name, tpl["raw"])
        print(f"[{idx}] 模板ID：{tpl_id} | 模板名称：{tpl_name}")

    # 保存到registry
    save_registry(REGISTRY_PATH, registry)
    print(f"\n=== 模板列表已保存到 {os.path.abspath(REGISTRY_PATH)} ===")
    print("你可以复制上面的模板ID到 MODE=2 的 TEMPLATE_ID 配置项中")


def run_mode_create_from_template(token: str, admin_userid: str) -> str:
    """MODE=2：基于模板创建新文档，获取ID并同步到registry"""
    if not TEMPLATE_ID.strip():
        raise RuntimeError("MODE=2 需要先配置 TEMPLATE_ID（可先运行 MODE=3 获取）")

    print("=== 基于模板创建新文档开始 ===")
    # 1. 基于模板创建新文档
    doc = create_doc_from_template(token, TEMPLATE_ID, TEMPLATE_CREATE_DOC_NAME, [admin_userid])
    new_docid = doc["docid"]
    new_doc_url = doc.get("url", "")

    print(f"基于模板创建文档成功：")
    print(f"模板ID：{TEMPLATE_ID}")
    print(f"新文档ID：{new_docid}")
    if new_doc_url:
        print(f"新文档链接：{new_doc_url}")

    # 2. 同步到registry
    registry = load_registry(REGISTRY_PATH)
    ensure_doc_entry(registry, new_docid, admin_userid, TEMPLATE_CREATE_DOC_NAME, new_doc_url)

    # 3. 获取新文档的工作表并同步
    cloud_sheets = get_sheets(token, new_docid)
    diff = sync_sheets_to_registry_with_diff(registry, new_docid, cloud_sheets)
    save_registry(REGISTRY_PATH, registry)

    print("\n=== registry 已更新 ===")
    print("registry_path:", os.path.abspath(REGISTRY_PATH))
    print_diff_log(1, 1, new_docid, TEMPLATE_CREATE_DOC_NAME, diff)

    maybe_pull_records(token, registry, new_docid)
    return new_docid
...
def run_mode_sync_all(token: str, admin_userid: str) -> None:
    ...
    if not docids:
        raise RuntimeError(
            ...
            f"解决：先 MODE=1/2 创建一次，或手动把 docid 写入 registry 后再运行 MODE=0。"
        )
...
def main():
    ...
    if MODE == 1:
        ...
        new_docid = run_mode_create_from_template(token, admin_userid)
        print("\n基于模板创建完成，新文档ID：", new_docid)
        print("新文档信息已存入 registry，可运行 MODE=0 刷新工作表信息。")

    elif MODE == 3:
        run_mode_get_templates(token)

    else:
        raise RuntimeError("MODE 只能是 0/1/2/3")
